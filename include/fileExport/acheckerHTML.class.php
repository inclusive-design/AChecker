...
<input value="Make Decisions" type="submit" name="make_decision" />
	...
	*/
	public function	getHTMLfile($problem, $_gids, $errors, $user_link_id) 
	{
		...
		$path = AC_TEMP_DIR.$filename.'.html';  
		$handle = fopen($path, 'w');	
	...
	{		
		if ($problem_type == 'known') {
			...
			if ($this->error_nr_likely == 0) {
				$content = "<ul><li class='msg_info'><span id='AC_congrats_msg_for_likely' class='congrats_msg'>
			...
			if ($this->error_nr_potential == 0) {
				$content = "<ul><li class='msg_info'><span id='AC_congrats_msg_for_potential' class='congrats_msg'>
