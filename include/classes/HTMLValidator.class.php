...
class HTMLValidator {
	...
	*/
	function HTMLValidator($type, $content)
	{
		...
		if ($this->validate_type == "uri")
		{
			...
			//MB solo se sto usando rest
			if (isset($_GET['output']) && $_GET['output']=='rest')
			{
			$this->realUrl;
			$dom=str_get_dom($result);
			$e=$dom->find("input[id=uri]");
			$this->realUrl=$e[0]->attr["value"];
			$dom->clear();
			}
		}
  ...
  * send post request and html content to 
  ...
  function do_post_request($url, $data, $optional_headers = null)
  {
	...
	* MB
	*  
	* restituisce il vero indirizzo della pagina validata ottenuto dal validatore w3c
	*/
	function getRealUrl()
	{
		//$dom=file_get_html($this->result);
		//$realUrl=$dom->find("input");
