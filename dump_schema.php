...
$lang_db = mysqli_connect($host, $mysql_user, $mysql_password) or die(mysqli_error($lang_db));
...
foreach ($dump_tables as $table_name) {
	...
	if (mysqli_num_rows($result) > 0) {
		...
		for ($i = 0; $i < mysqli_num_fields($result); ++$i) {
		    $field_fetch = mysqli_fetch_field_direct($result, $i)
		    $field_name = $field_fetch->name;
		    $field_type = $field_fetch->type;
