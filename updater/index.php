...
if (true === defined('AC_PROXY_HOST'))
{
	$proxy_connection = array(
		'http' => array(
			'proxy' => 'tcp://' . AC_PROXY_HOST . ':' . AC_PROXY_PORT,
			'request_fulluri' => true,
		),
	);
	if (true === defined('AC_PROXY_USER'))
	{
		$proxy_auth = base64_encode(AC_PROXY_USER . ':' . AC_PROXY_PASS);
		$proxy_connection['http']['header'] = 'Proxy-Authorization: Basic ' . $proxy_auth;
	}
	$connection_context = stream_context_create($proxy_connection);
	$connection = file_get_contents($connection_test_file, false, $connection_context);
}
else {
...
if (true === $server_connected)
{
	$patch_folder = $update_server . '/achecker/patch/' . str_replace('.', '_', VERSION) . '/';
	$patch_list_xml = (defined('AC_PROXY_HOST')) ? @file_get_contents($patch_folder.'patch_list.xml', false, $connection_context) : @file_get_contents($patch_folder . 'patch_list.xml');
...
if ($_POST['install'] || $_POST['install_upload'] && !isset($_POST["not_ignore_version"]))
{
	...
	else
	{
		...
		$patch_xml = (defined('AC_PROXY_HOST')) ? file_get_contents($patchURL.'patch.xml', false, $connection_context) : file_get_contents($patchURL . 'patch.xml');
